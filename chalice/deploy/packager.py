...
class SDistMetadataFetcher(object):
    ...
    def get_package_name_and_version(self, sdist_path: str) -> Tuple[str, str]:
        with self._osutils.tempdir() as tempdir:
            ...
            try:
                ...
                parsed = self._parse_from_pyproject_toml(package_dir)
                if parsed is None:
                    raise
                name, version = parsed
        return name, version
    ...
    def _parse_from_pyproject_toml(
        self,
        package_dir: str,
    ) -> Optional[Tuple[str, str]]:
        # As a last attempt fallback, if we don't have a PKG-INFO file, we'll
        # try to parse directly from a pyproject.toml if it exists.
        pyproject_toml = self._osutils.joinpath(package_dir, 'pyproject.toml')
        if self._osutils.file_exists(pyproject_toml):
